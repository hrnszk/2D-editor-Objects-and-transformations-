...
class App(val canvas : HTMLCanvasElement, val overlay : HTMLDivElement) {
  ...
  fun registerEventHandlers() {
    ...
    canvas.onmousedown = { 
      event : MouseEvent ->
      mouseDown = true
      scene.oldMousePos.x = 2 * event.x.toFloat()/canvas.width - 1f
      scene.oldMousePos.y = -(2 * event.y.toFloat()/canvas.height - 1f)
    ...
    canvas.onmousemove = { 
      ...
      if(mouseDown){
        ...
        // scene.newMousePos.x = 2 * mX/canvas.width
        // scene.newMousePos.y = (2 * mY/canvas.height) * (-1)
      }
